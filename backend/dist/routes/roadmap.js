...
router.post('/generate', authenticate, async (req, res, next) => {
    try {
        const { targetRole, targetCompany, durationWeeks = 8, currentSkills = [] } = req.body;
        if (!targetRole)
        ...
        const weeks = Array.from({ length: Math.min(durationWeeks, 4) }, (_, i) => ({
            ...
            tasks: [
                { id: `t${i}1`, text: 'Study core concepts', type: 'learn', done: false, estimatedHours: 5 },
                { id: `t${i}2`, text: 'Build a project', type: 'build', done: false, estimatedHours: 8 },
            ],
            resources: [],
            completed: false,
        }));
...
router.patch('/:id/week/:weekId/task', authenticate, async (req, res, next) => {
    try {
