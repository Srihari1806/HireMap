...
router.get('/', async (req: Request, res: Response, next) => {
  try {
    const {
      page = '1', limit = '20', type, domain, location,
      remote, search, source, sortBy,
    ...
    const orderBy: Record<string, string> =
      sortBy === 'salary' ? { salaryMax: 'desc' } : { postedAt: 'desc' };
...
router.get('/stats/overview', async (_req: Request, res: Response, next) => {
  try {
...
router.get('/user/applications', authenticate, async (req: AuthRequest, res: Response, next) => {
  try {
