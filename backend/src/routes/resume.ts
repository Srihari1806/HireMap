...
router.post('/upload', authenticate, upload.single('resume'), async (req: AuthRequest, res: Response, next) => {
  try {
...
router.post('/ats-check', authenticate, async (req: AuthRequest, res: Response, next) => {
  try {
...
router.get('/ats-history', authenticate, async (req: AuthRequest, res: Response, next) => {
  try {
...
// GET /api/resume/list
router.get('/list', authenticate, async (req: AuthRequest, res: Response, next) => {
  try {
...
router.post('/enhance-section', authenticate, async (req: AuthRequest, res: Response, next) => {
  try {
    const { section, content } = req.body as { section: string; content: string; targetRole?: string };
    if (!section || !content) throw new AppError('section and content are required', 400);
...
router.post('/cover-letter', authenticate, async (req: AuthRequest, res: Response, next) => {
  try {
    const { targetRole, company } = req.body as {
      targetRole: string; company: string; tone?: string;
    };
    if (!targetRole || !company) throw new AppError('targetRole and company are required', 400);
